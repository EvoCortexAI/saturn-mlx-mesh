...
- Phase 8 minimal Runtime DAG executor skeleton: `RuntimeStage` protocol, `RuntimeDAG`, `MeshGraphExecutor` actor, `StageResult`/`StageSideEffect`; `withTaskGroup` + deterministic merge (declaration order) + telemetry hooks via existing `MeshTelemetry`; one self-contained local demo DAG (two independent stages + merge) runs in the new `testL8MinimalRuntimeDAGSkeletonAndOneDemoDAG` (pure Swift, sim-safe, exercises all 7 required items, 14/14 tests green). The landed L8 modeling (MeshStage / graph stages / sketch) remains fully compatible. Per the explicit minimal approval: no Phoenix/Thunder/Grox, no real graph rewriting / KV integration / remote, no new public session API, adapters/inspirations only in comments. This is the executable foundation for L9/L10 scheduler rewrite over the weighted computation graph.
